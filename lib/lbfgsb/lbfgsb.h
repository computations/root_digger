...
#ifndef abs
#define LBFGSB_ABS(x) ((x) >= 0 ? (x) : -(x))
#else
#define LBFGSB_ABS abs
#endif
#ifndef min
#define LBFGSB_MIN(a, b) ((a) <= (b) ? (a) : (b))
#else
#define LBFGSB_MIN min
#endif
#ifndef max
#define LBFGSB_MAX(a, b) ((a) >= (b) ? (a) : (b))
#else
#define LBFGSB_MAX max
#endif
...
#define IS_CONVERGED(x)                                                        \
  (((x) >= CONVERGENCE) ? (((x) <= CONVERGENCE_END) ? 1 : 0) : 0)
#define CONV_GRAD                                                              \
  21 /* message: CONVERGENCE: NORM_OF_PROJECTED_GRADIENT_<=_PGTOL. */
#define CONV_F                                                                 \
  22 /* message: CONVERGENCE: REL_REDUCTION_OF_F_<=_FACTR*EPSMCH. */
...
#define STOP_ITER                                                              \
  32 /* message: STOP: TOTAL NO. of f AND g EVALUATIONS EXCEEDS LIM.  */
#define STOP_GRAD                                                              \
  33 /* message: STOP: THE PROJECTED GRADIENT IS SUFFICIENTLY SMALL. */
...
extern int dcopy(int *, double *, int *, double *, int *);

extern int setulb(int *n, int *m, double *x, double *l, double *u, int *nbd,
                  double *f, double *g, double *factr, double *pgtol,
                  double *wa, int *iwa, int *task, int *iprint, int *csave,
                  logical *lsave, int *isave,
                  double *dsave); /* ftnlen task_len, ftnlen csave_len); */

extern int mainlb(int *n, int *m, double *x, double *l, double *u, int *nbd,
                  double *f, double *g, double *factr, double *pgtol,
                  double *ws, double *wy, double *sy, double *ss, double *wt,
                  double *wn, double *snd, double *z__, double *r__,
                  double *d__, double *t, double *xp, double *wa, int *index,
                  int *iwhere, int *indx2, int *task, int *iprint, int *csave,
                  logical *lsave, int *isave, double *dsave);

extern int freev(int *, int *, int *, int *, int *, int *, int *, logical *,
                 logical *, logical *, int *, int *);

extern int formk(int *, int *, int *, int *, int *, int *, int *, logical *,
                 double *, double *, int *, double *, double *, double *,
                 double *, int *, int *, int *);

extern int formt(int *, double *, double *, double *, int *, double *, int *);

extern int subsm(int *, int *, int *, int *, double *, double *, int *,
                 double *, double *, double *, double *, double *, double *,
                 double *, double *, int *, int *, int *, double *, double *,
                 int *, int *);

extern int active(int *, double *, double *, int *, double *, int *, int *,
                  logical *, logical *, logical *);

extern int cauchy(int *, double *, double *, double *, int *, double *, int *,
                  int *, double *, double *, double *, int *, double *,
                  double *, double *, double *, double *, int *, int *,
                  double *, double *, double *, double *, int *, int *,
                  double *, int *, double *);

extern int cmprlb(int *, int *, double *, double *, double *, double *,
                  double *, double *, double *, double *, double *, int *,
                  double *, int *, int *, int *, logical *, int *);

extern int matupd(int *, int *, double *, double *, double *, double *,
                  double *, double *, int *, int *, int *, int *, double *,
                  double *, double *, double *, double *);

extern int lnsrlb(int *n, double *l, double *u, int *nbd, double *x, double *f,
                  double *fold, double *gd, double *gdold, double *g,
                  double *d__, double *r__, double *t, double *z__, double *stp,
                  double *dnorm, double *dtd, double *xstep, double *stpmx,
                  int *iter, int *ifun, int *iback, int *nfgv, int *info,
                  int *task, logical *boxed, logical *cnstnd, int *csave,
                  int *isave, double *dsave); /* ftnlen task_len, ftnlen
           csave_len); */

extern int projgr(int *, double *, double *, int *, double *, double *,
                  double *);

/* in linesearch.c */
extern int dcsrch(double *f, double *g, double *stp, double *ftol, double *gtol,
                  double *xtol, double *stpmin, double *stpmax, int *task,
                  int *isave, double *dsave); /* ftnlen task_len);*/

extern int dcstep(double *, double *, double *, double *, double *, double *,
                  double *, double *, double *, logical *, double *, double *);

extern int dpofa(double *, int *, int *, int *);

extern int dtrsl(double *, int *, int *, double *, int *, int *);
