...
#ifdef DEBUG
  if (*prjctd) {
    DBG("[L-BFGS-B] The initial X is infeasible. Restart with its "
        "projection\n");
  }
  if (!(*cnstnd)) {
...
int cauchy(int *n, double *x, double *l, double *u, int *nbd, double *g,
           int *iorder, int *iwhere, double *t, double *d__, double *xcp,
           int *m, double *wy, double *ws, double *sy, double *wt,
           double *theta, int *col, int *head, double *p, double *c__,
           double *wbp, double *v, int *nseg, int *iprint, double *sbgnrm,
           int *info, double *epsmch) {

  /* System generated locals */
  int wy_dim1, wy_offset, ws_dim1, ws_offset, sy_dim1, sy_offset, wt_dim1,
      wt_offset, i__1, i__2;
  double d__1;
...
L777:
  ...
  /* ------------------- the end of the loop ------------------------------- */
L888:
  DBG("[L-BFGS-B] \nGCP found in this segment. Piece %d --f1, f2 at start "
      "point %.2e %.2e\n",
      *nseg, f1, f2);
  DBG("[L-BFGS-B] Distance to the stationary point = %.2e\n", dtm);
  if (dtm <= 0.) {
...
int cmprlb(int *n, int *m, double *x, double *g, double *ws, double *wy,
           double *sy, double *wt, double *z__, double *r__, double *wa,
           int *index, double *theta, int *col, int *head, int *nfree,
           logical *cnstnd, int *info) {
  /* System generated locals */
  int ws_dim1, ws_offset, wy_dim1, wy_offset, sy_dim1, sy_offset, wt_dim1,
      wt_offset, i__1, i__2;
  ...
  if (!(*cnstnd) && *col > 0) {
    ...
    bmv(m, &sy[sy_offset], &wt[wt_offset], col, &wa[(*m << 1) + 1], &wa[1],
        info);
    if (*info != 0) {
      *info = -8;
      return 0;
    ...
    for (j = 1; j <= i__1; ++j) {
      ...
      for (i = 1; i <= i__2; ++i) {
        k = index[i];
        r__[i] = r__[i] + wy[k + pointr * wy_dim1] * a1 +
...
/* ======================= The end of cmprlb ============================= */
int formk(int *n, int *nsub, int *ind, int *nenter, int *ileave, int *indx2,
          int *iupdat, logical *updatd, double *wn, double *wn1, int *m,
          double *ws, double *wy, double *sy, double *theta, int *col,
          int *head, int *info) {
  /* System generated locals */
  int wn_dim1, wn_offset, wn1_dim1, wn1_offset, ws_dim1, ws_offset, wy_dim1,
      wy_offset, sy_dim1, sy_offset, i__1, i__2, i__3;
  ...
  for (is = *col + 1; is <= i__1; ++is) {
    ...
    for (js = is; js <= i__2; ++js) {
      wn[is + js * wn_dim1] +=
          ddot(col, &wn[is * wn_dim1 + 1], &c__1, &wn[js * wn_dim1 + 1], &c__1);
      /* L74: */
    }
    /* L72: */
  }
...
int formt(int *m, double *wt, double *sy, double *ss, int *col, double *theta,
          int *info) {
  /* System generated locals */
  int wt_dim1, wt_offset, sy_dim1, sy_offset, ss_dim1, ss_offset, i__1, i__2,
      i__3;
  ...
  for (i__ = 2; i__ <= i__1; ++i__) {
    i__2 = *col;
    for (j = i__; j <= i__2; ++j) {
      k1 = LBFGSB_MIN(i__, j) - 1;
      ddum = 0.;
      i__3 = k1;
      for (k = 1; k <= i__3; ++k) {
        ddum +=
            sy[i__ + k * sy_dim1] * sy[j + k * sy_dim1] / sy[k + k * sy_dim1];
        /* L53: */
      }
...
int freev(int *n, int *nfree, int *index, int *nenter, int *ileave, int *indx2,
          int *iwhere, logical *wrk, logical *updatd, logical *cnstnd,
          int *iprint, int *iter) {
  /* System generated locals */
  int i__1;
...
int matupd(int *n, int *m, double *ws, double *wy, double *sy, double *ss,
           double *d__, double *r__, int *itail, int *iupdat, int *col,
  ...
  *theta = *rr / *dr;
  ...
  if (*iupdat > *m) {
    ...
    for (j = 1; j <= i__1; ++j) {
      ...
      i__2 = *col - j;
      dcopy(&i__2, &sy[j + 1 + (j + 1) * sy_dim1], &c__1, &sy[j + j * sy_dim1],
            &c__1);
      /* L50: */
    }
  }
  ...
  for (j = 1; j <= i__1; ++j) {
    sy[*col + j * sy_dim1] =
        ddot(n, &d__[1], &c__1, &wy[pointr * wy_dim1 + 1], &c__1);
    ss[j + *col * ss_dim1] =
        ddot(n, &ws[pointr * ws_dim1 + 1], &c__1, &d__[1], &c__1);
    pointr = pointr % *m + 1;
    /* L51: */
  }
  if (*stp == 1.) {
...
int projgr(int *n, double *l, double *u, int *nbd, double *x, double *g,
           double *sbgnrm) {
  /* System generated locals */
  int i__1;
  double d__1, d__2;
    ...
    if (nbd[i__] != 0) {
      if (gi < 0.) {
        if (nbd[i__] >= 2) {
          /* Computing MAX */
          d__1 = x[i__] - u[i__];
          gi = LBFGSB_MAX(d__1, gi);
        }
      } else {
        if (nbd[i__] <= 2) {
          /* Computing MIN */
          d__1 = x[i__] - l[i__];
          gi = LBFGSB_MIN(d__1, gi);
        }
      }
    }
    /* Computing MAX */
    d__1 = *sbgnrm, d__2 = LBFGSB_ABS(gi);
    *sbgnrm = LBFGSB_MAX(d__1, d__2);
    /* L15: */
  }
  return 0;
} /* projgr */
...
int subsm(int *n, int *m, int *nsub, int *ind, double *l, double *u, int *nbd,
          double *x, double *d__, double *xp, double *ws, double *wy,
          double *theta, double *xx, double *gg, int *col, int *head,
          int *iword, double *wv, double *wn, int *iprint, int *info) {
  ...
  /*      "Remark On Algorithm 788: L-BFGS-B: Fortran Subroutines for
   * Large-Scale */
    ...
    if (nbd[k] != 0) {
      ...
      if (nbd[k] == 1) {
        ...
        if (nbd[k] == 2) {
          ...
          /* Computing MAX */
          d__1 = l[k], d__2 = xk + dk;
          xk = LBFGSB_MAX(d__1, d__2);
          /* Computing MIN */
          d__1 = u[k];
          x[k] = LBFGSB_MIN(d__1, xk);
          if (x[k] == l[k] || x[k] == u[k]) {
            *iword = 1;
          }
        } else {
