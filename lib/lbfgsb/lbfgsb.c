...
static int prn2lb(int *n, double *x, double *f, double *g, int *iprint,
                  int *iter, int *nfgv, int *nact, double *sbgnrm, int *nseg,
                  int *word, int *iword, int *iback, double *stp, double *xstep,
                  ftnlen word_len);
static int prn3lb(int *n, double *x, double *f, int *task, int *iprint,
                  ...
                  ftnlen task_len, ftnlen word_len);
#endif
...
int setulb(int *n, int *m, double *x, double *l, double *u, int *nbd, double *f,
           double *g, double *factr, double *pgtol, double *wa, int *iwa,
           int *task, int *iprint, int *csave, logical *lsave, int *isave,
           double *dsave) /* ftnlen task_len, ftnlen csave_len) */
{
  ...
  lxp = isave[15];
  lwa = isave[16];
  mainlb(n, m, &x[1], &l[1], &u[1], &nbd[1], f, &g[1], factr, pgtol, &wa[lws],
         &wa[lwy], &wa[lsy], &wa[lss], &wa[lwt], &wa[lwn], &wa[lsnd], &wa[lz],
         &wa[lr], &wa[ld], &wa[lt], &wa[lxp], &wa[lwa], &iwa[1], &iwa[*n + 1],
         &iwa[(*n << 1) + 1], task, iprint, csave, &lsave[1], &isave[22],
         &dsave[1]); /* (ftnlen)60, (ftnlen)60); */
  return 0;
} /* setulb_ */
...
int mainlb(int *n, int *m, double *x, double *l, double *u, int *nbd, double *f,
           double *g, double *factr, double *pgtol, double *ws, double *wy,
           double *sy, double *ss, double *wt, double *wn, double *snd,
           double *z__, double *r__, double *d__, double *t, double *xp,
           double *wa, int *index, int *iwhere, int *indx2, int *task,
           int *iprint, int *csave, logical *lsave, int *isave,
           double *dsave) /*(ftnlen ) */
    ...
    *word = WORD_DEFAULT;
    ...
    if (IS_ERROR(*task)) {
      ...
      prn3lb(n, &x[1], f, task, iprint, &info, &iter, &nfgv, &nintol, &nskip,
             &nact, &sbgnrm, &c_b7, &nseg, word, &iback, &stp, &xstep, &k,
...
L222:
  ...
  freev(n, &nfree, &index[1], &nenter, &ileave, &indx2[1], &iwhere[1], &wrk,
...
L333:
  ...
  if (nfree && col) {
    ...
    /* [ 0  I] */
    if (wrk) {
      formk(n, &nfree, &index[1], &nenter, &ileave, &indx2[1], &iupdat, &updatd,
            &wn[wn_offset], &snd[snd_offset], m, &ws[ws_offset], &wy[wy_offset],
            &sy[sy_offset], &theta, &col, &head, &info);
    }
    if (info != 0) {
      DBG("[L-BFGS-B]  Nonpositive definiteness in Cholesky factorization in "
          "formk;\n");
      DBG("[L-BFGS-B]    refresh the lbfgs memory and restart the "
          "iteration.\n");
      info = 0;
      col = 0;
      head = 1;
  ...
  timer(&cpu1);
L666:
  lnsrlb(n, &l[1], &u[1], &nbd[1], &x[1], f, &fold, &gd, &gdold, &g[1], &d__[1],
         &r__[1], &t[1], &z__[1], &stp, &dnorm, &dtd, &xstep, &stpmx, &iter,
         &ifun, &iback, &nfgv, &info, task, &boxed, &cnstnd, csave, &isave[22],
         &dsave[17]); /* (ftnlen)60, (ftnlen)60); */
  if (info != 0 || iback >= 20) {
    /* restore the previous iterate. */
    dcopy(n, &t[1], &c__1, &x[1], &c__1);
    dcopy(n, &r__[1], &c__1, &g[1], &c__1);
    *f = fold;
    ...
    prn2lb(n, &x[1], f, &g[1], iprint, &iter, &nfgv, &nact, &sbgnrm, &nseg,
           word, &iword, &iback, &stp, &xstep, (ftnlen)3);
#endif
    goto L1000;
  }
L777:
  ...
  /* Computing MAX */
  d__1 = LBFGSB_ABS(fold), d__2 = LBFGSB_ABS(*f), d__1 = LBFGSB_MAX(d__1, d__2);
  ddum = LBFGSB_MAX(d__1, 1.);
  if (fold - *f <= tol * ddum) {
    /* terminate the algorithm. */
    *task = CONV_F;
    if (iback >= 10) {
  ...
  matupd(n, m, &ws[ws_offset], &wy[wy_offset], &sy[sy_offset], &ss[ss_offset],
         &d__[1], &r__[1], &itail, &iupdat, &col, &head, &theta, &rr, &dr, &stp,
         &dtd);
...
L999:
  ...
  prn3lb(n, &x[1], f, task, iprint, &info, &iter, &nfgv, &nintol, &nskip, &nact,
         &sbgnrm, &cur_time, &nseg, word, &iback, &stp, &xstep, &k, &cachyt,
         &sbtime, &lnscht, (ftnlen)60, (ftnlen)3);
#endif
L1000:
 ...
 */
int prn2lb(int *n, double *x, double *f, double *g, int *iprint, int *iter,
           int *nfgv, int *nact, double *sbgnrm, int *nseg, int *word,
...
int prn3lb(int *n, double *x, double *f, int *task, int *iprint, int *info,
           int *iter, int *nfgv, int *nintol, int *nskip, int *nact,
           double *sbgnrm, double *time, int *nseg, int *word, int *iback,
           double *stp, double *xstep, int *k, double *cachyt, double *sbtime,
  ...
  if (!IS_ERROR(*task)) {
    if (*iprint >= 0) {
      ...
      printf("Nact  = number of active bounds at final generalized Cauchy "
             "point\n");
  ...
  if (*iprint >= 0) {
    ...
    if (*info != 0) {
      if (*info == -1)
        printf(" Matrix in 1st Cholesky factorization in formk is not Pos. "
               "Def.\n");
      if (*info == -2)
        printf(" Matrix in 2nd Cholesky factorization in formk is not Pos. "
               "Def.\n");
      if (*info == -3)
        printf(" Matrix in the Cholesky factorization in formt is not Pos. "
               "Def.\n");
      if (*info == -4) {
